...
export default function DesignToCodeSection() {
  ...
  // Start with both visible and active right away
  const scaleLeft = useTransform(scrollYProgress, [0, 0.25], [1.05, 1]);
  const xLeft = useTransform(scrollYProgress, [0, 0.2, 0.8, 1], ["0%", "0%", "0%", "-3%"]);
  
  // Right code panel is immediately visible (starts at opacity 0.85 and goes to 1 right away)
  const opacityRight = useTransform(scrollYProgress, [0, 0.12], [0.85, 1]);
  const xRight = useTransform(scrollYProgress, [0, 0.15, 0.8, 1], ["4%", "0%", "0%", "3%"]);
  
  // Highlight triggers quickly in the middle of active scroll
  const highlightOpacity = useTransform(scrollYProgress, [0.18, 0.3, 0.65, 0.8], [0, 1, 1, 0]);

  // Smooth exit transition
  const sectionOpacity = useTransform(scrollYProgress, [0.88, 1], [1, 0.3]);

  return (
    <section ref={targetRef} className="relative h-[200vh] bg-dark">
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden bg-grain">
        ...
        <motion.div style={{ opacity: sectionOpacity }} className="w-full">
          <div className="absolute top-16 md:top-20 left-6 md:left-12 z-20">
          ...
          <div className="container mx-auto px-6 md:px-12 flex flex-col md:flex-row h-[60vh] mt-24 relative z-10 gap-6 md:gap-0">
