...
export default function ResponsiveSection() {
  ...
  const desktopScale = useTransform(scrollYProgress, [0, 0.4], [1, 0.6]);
  const desktopY = useTransform(scrollYProgress, [0, 0.4], ["0%", "-10%"]);
  const desktopOpacity = useTransform(scrollYProgress, [0, 0.4], [1, 0.5]);

  const tabletOpacity = useTransform(scrollYProgress, [0.3, 0.5], [0, 1]);
  const tabletX = useTransform(scrollYProgress, [0.3, 0.5], ["10%", "-30%"]);
  const tabletY = useTransform(scrollYProgress, [0.3, 0.5], ["20%", "10%"]);

  const mobileOpacity = useTransform(scrollYProgress, [0.6, 0.8], [0, 1]);
  const mobileX = useTransform(scrollYProgress, [0.6, 0.8], ["20%", "40%"]);
  const mobileY = useTransform(scrollYProgress, [0.6, 0.8], ["30%", "20%"]);
  ...
  return (
    <section ref={containerRef} className="relative h-[300vh] bg-[#050606] border-t border-neutral-900">
      <div className="sticky top-0 h-screen flex flex-col items-center justify-center overflow-hidden bg-grain">
        ...
        <div className="relative w-full max-w-5xl h-[60vh] mt-32 flex items-center justify-center">
          
          {/* Desktop */}
          <motion.div 
            style={{ scale: desktopScale, y: desktopY, opacity: desktopOpacity }}
            className="absolute w-[800px] h-[500px] border border-neutral-800 bg-[#080909] shadow-2xl flex flex-col z-10"
          >
          ...
          <motion.div 
            ...
            className="absolute w-[400px] h-[550px] border border-neutral-800 bg-[#080909] shadow-2xl flex flex-col z-20"
          >
          ...
          <motion.div 
            ...
            className="absolute w-[250px] h-[500px] border border-neutral-800 bg-[#080909] shadow-2xl rounded-[2rem] flex flex-col overflow-hidden z-30"
          >
