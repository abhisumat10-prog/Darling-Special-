...
| **Person 3** | Backend + AI Agent | Serverless `/grade` endpoint calling Gemini 3.6/ API | Use system prompt & rubric in `prompts/grading-rubric-prompt.md`; validate against `contracts/grade-request.schema.json` and respond with `contracts/grade-response.schema.json` |
